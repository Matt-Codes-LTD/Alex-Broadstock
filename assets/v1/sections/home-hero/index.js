...
export default function initHomeHero(container) {
  ...
  links.forEach((lnk, i) => {
    ...
    if (v) i < MAX_EAGER ? videoManager.warmVideo(v) : (window.requestIdleCallback || ((fn) => setTimeout(fn, 400)))(() => videoManager.warmVideo(v));
  });
  ...
  const visHandler = () => {
    if (document.hidden) stage.querySelectorAll(".home-hero_video_el").forEach((v) => v.pause?.());
  };
