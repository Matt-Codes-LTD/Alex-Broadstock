...
export default function initProjectPlayer(container) {
  ...
  async function ensureFirstFramePainted(v) {
    ...
    try { "fastSeek" in v ? v.fastSeek(0) : (v.currentTime = 0.00001); } catch {}
    try { v.muted = true; v.setAttribute("muted", ""); await v.play?.().catch(() => {}); } catch {}
    await new Promise((res) => {
      let done = false, cap = setTimeout(() => { if (!done) { done = true; res(); } }, 800);
      v.addEventListener("timeupdate", () => { if (!done) { done = true; clearTimeout(cap); requestAnimationFrame(res); } }, { once: true });
    });
    try { v.pause(); } catch {}
  }
  ...
  function seekPct(p) { if (!isFinite(video.duration)) return; video.currentTime = (Math.max(0, Math.min(100, p)) / 100) * video.duration; updateTimeUI(); }
    ...
    try {
      if (video.paused) {
        if (video.muted || video.volume === 0) {
          ...
          setMuteUI(false); didFirstSoundRestart = true;
        }
        await video.play();
  ...
  const centerHandler = async () => {
    ...
    if (!inPlayMode) {
      try {
        ...
        video.currentTime = 0;
        await video.play?.();
        switchCenterToPlayMode(); setMuteUI(false); setPlayUI(true); setPausedUI(false);
  ...
  const muteHandler = async () => {
    ...
    if (video.muted) {
      ...
      if (wasMuted && !didFirstSoundRestart) {
        didFirstSoundRestart = true;
        video.currentTime = 0; await video.play?.(); setPlayUI(true); setPausedUI(false);
        switchCenterToPlayMode();
      }
    }
    localStorage.setItem("pp:muted", video.muted ? "1" : "0");
    setMuteUI(video.muted); kickHide();
  };
  ...
  handlers.push(() => document.removeEventListener("fullscreenchange", updateFSLabel));
  ...
  // Timeline
  if (tl) {
    const onDown = (e) => { dragging = true; tl.setPointerCapture?.(e.pointerId); const r = tl.getBoundingClientRect(); seekPct(((e.clientX - r.left) / r.width) * 100); kickHide(); };
    const onMove = (e) => { if (dragging) { const r = tl.getBoundingClientRect(); seekPct(((e.clientX - r.left) / r.width) * 100); } };
    ...
    handlers.push(() => { tl.removeEventListener("pointerdown", onDown); tl.removeEventListener("pointermove", onMove); tl.removeEventListener("pointerup", endDrag); tl.removeEventListener("pointercancel", endDrag); tl.removeEventListener("keydown", onKey); });
  }
  ...
  // Keep UI in sync
  video.addEventListener("play", () => { setPlayUI(true); setPausedUI(false); });
  video.addEventListener("playing", () => setPausedUI(false));
  video.addEventListener("pause", () => { setPlayUI(false); setPausedUI(true); });
  video.addEventListener("ended", () => { setPlayUI(false); setPausedUI(true); });
  video.addEventListener("timeupdate", () => { if (!dragging) updateTimeUI(); });
  handlers.push(() => {
  ...
  // Cleanup
  return () => {
    try { cancelAnimationFrame(raf); video.pause(); video.muted = true; } catch {}
