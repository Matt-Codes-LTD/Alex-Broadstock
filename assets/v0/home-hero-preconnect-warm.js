(() => {
  ...
  // Preconnect the CDN immediately
  preconnectOnce('https://alexbroadstock.b-cdn.net/');

  // Optional: warm the first hovered video element ON THIS PAGE ONLY (no cross-page adoption)
  function warmOnHover(root) {
    ...
    async function prime(url) {
      ...
      try {
        ...
        // Kick a frame to GPU, then pause
        const p = v.play?.(); if (p && p.then) await p.catch(()=>{});
        setTimeout(()=>{ try{ v.pause(); }catch(_){} }, 30);
        warmedVideo = v; // not adopted across pages; just keeps connection hot
      } catch(_) {}
    }
