...
document.addEventListener("DOMContentLoaded", () => {
  document.querySelectorAll(".home-hero_wrap").forEach((section) => {
    ...
    if (links.length) {
      ...
      // Don't visually switch until intro finishes; just prep
      stage.querySelectorAll(".home-hero_video_el.is-active").forEach(el=>el.classList.remove("is-active"));
      v.classList.add("is-active");
      try { v.play?.(); } catch(_) {}
    }

    // Gate pointer interactions until after intro
    let interactionsEnabled = reduceMotion; // if reduced-motion, enable immediately
    function enableInteractions(){
      if (interactionsEnabled) return;
      interactionsEnabled = true;
      // reattach listeners now
    ...
    window.addEventListener("home-intro:done", enableInteractions, { once:true });
    if (interactionsEnabled) enableInteractions(); // for reduced motion
