...
function initHomeHero(container) {
  ...
  const fadeTargetsFor = (link) =>
    link.querySelectorAll(link.getAttribute("data-fade-target") || defaultFadeSel);
    ...
    requestAnimationFrame(() => {
      ...
      Promise.allSettled(anims).finally(() => {
        ...
        if (
          ...
          document.activeElement.closest(".home-hero_list")?.style.display ===
            "none" &&
          firstVisibleLink
        ) {
  ...
  (function setupCategoriesFromNav() {
    ...
    function ensureAllButton() {
      ...
      let allBtn =
        buttons.find((b) => normalize(b.textContent) === "all") || null;
      if (!allBtn) {
        const item = document.createElement("div");
        item.setAttribute("role", "listitem");
        item.className =
          "home-hero_category u-text-style-main w-dyn-item";
    ...
    section.__catCleanup = () =>
      catWrap.removeEventListener("click", onClick);
  })();
...
document.head.appendChild(t);
  ...
  const visHandler = () => {
    if (document.hidden) {
      stage
        .querySelectorAll(".home-hero_video_el")
        .forEach((v) => {
          try {
            v.pause();
          } catch (_) {}
